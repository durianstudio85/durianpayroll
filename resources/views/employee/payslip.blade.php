			    ...
			    <table width="100%" class="table table-striped table-hover">
			        ...
			        <tbody>
			        	@foreach( $employee->payslips as $payslip)
				        	<tr>
				        		<td>{{ date('l, F d, Y', strtotime($payslip->payroll->date_start_range)) }} - {{ date('l, F d, Y', strtotime($payslip->payroll->date_end_range)) }}</td>
                                <td>{{ round(abs(strtotime($payslip->payroll->date_start_range)-strtotime($payslip->payroll->date_end_range))/86400) }} Days</td>
                                <td>{{ $payslip->payroll->status }}</td>
            ...
            <div class="modal-content">
                <div class="modal-header">
                 ...
                 <div class="modal-body">
                    <center>
                        @if( $employee->company->company_logo != '' )
                            <img src="{{ asset('upload/'.$employee->company->company_logo) }}" style="max-height: 60px; margin-bottom: 20px;">
                        @endif
                        
                        <p>{{ $employee->company->business_address }}</p>
                        @if ( $employee->company->contact_telephone != '' )
                            <p>Tel: {{ $employee->company->contact_telephone }}</p>
                        @endif
                    </center>
                    <hr>
                    <div class="row">
                        <div class="col-sm-6 col-xs-12">
                            ...
                            <div class='row'>
                                ...
                                <div class="col-xs-7"><p>{{ $employee->address }}</p></div>
                            </div>
                            <div class='row'>
                        ...
                        <div class="col-sm-6 col-xs-12">
                            <div class='row'>
                                ...
                                <div class="col-xs-7"><p>{{ date('l, F d, Y', strtotime($payslip->payroll->date_end_range)) }}</p></div>
                            </div>
                            <div class='row'>
                                <div class="col-xs-5"><p>Employee SSN:</p></div>
                                <div class="col-xs-7"><p>{{$employee->ssn }}</p></div>
                            </div>
                            <div class='row'>
                                <div class="col-xs-5"><p>Mode of Payment:</p></div>
                                <div class="col-xs-7"><p>{{ $employee->payment_mode }}</p></div>
                            </div>
                        </div>
                    </div>
                    <hr>
